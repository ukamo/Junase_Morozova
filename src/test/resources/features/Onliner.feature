...
Feature: Add some product in Cart in application Onliner
  ...
  Scenario Outline: Add some product in Cart in application Onliner
    ...
    Then I assert popover <popover name> is displayed on screen <product name> in application Onliner
    ...
    Then I assert header Товар добавлен в корзину is present on sidebar on screen <product name> in application Onliner
    ...
    Examples:
      ...
      |HONOR Choice Moecen Earbuds X3 Lite (международная версия)| Ваш населенный пункт — Минск? |